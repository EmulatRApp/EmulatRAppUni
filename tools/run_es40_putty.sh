#!/usr/bin/env bash
# ============================================================================
# tools/run_es40_putty.sh -- cold-boot the ES40 firmware to P00>>> with a
...
# ============================================================================
#
# FILE: tools/run_es40_putty.sh
# FUNCTION: ES40 sibling of run_ds20_putty.sh.  Written 2026-08-05 to complete
...
#              ES40 firmware AND the ES40 manifest) rather than hardcoded to
...
FW_REL="firmware/es40_v7_3.exe"
MANIFEST="es40_v7_3_platform.json"

# ---- auto-pick the newest run dir that can actually boot ES40 ---------------
RUN_DIR=""
NEWEST=0
...
# ---- force model=ES40 for this run; restore on exit -------------------------
cleanup() {
...
if [[ -f "$INI" ]]; then
    ...
    if grep -qiE '^[[:space:]]*model[[:space:]]*=' "$INI"; then
        ...
        SED_TMP="$(mktemp)"
        sed "s/^\([[:space:]]*model[[:space:]]*=[[:space:]]*\).*/\1ES40/" "$INI" > "$SED_TMP" \
            && mv -f "$SED_TMP" "$INI"
    else
        echo "WARN: no model= line in $INI -- relying on the firmware stem for ES40"
    fi
fi
...
if grep -qE 'P00>>>' "$LOG"; then
    echo "GATE   : P00>>> REACHED -- the ES40 leg passes."
else
    echo "GATE   : P00>>> NOT seen in the log -- the ES40 leg FAILS."
fi
