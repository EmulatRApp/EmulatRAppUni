...
TO BE FILLED BY COWORK:

    4.4   pre-LFU .rom backup?      -> EXISTS at ... / DOES NOT EXIST   <-- FIRST
    E-0.1 toFaultCode(ItbMiss)      -> ...
    E-0.2 consumer of :556          -> ...
    E-0.3 PalBox ITB_MISS dispatch  -> ...
    E-2.1 flash seeding policy      -> seed-if-absent / seed-always / other
                                       (D-2 predicts seed-if-absent; confirm)
    E-2.2 .rom path, size, mtime    -> ...
    E-2.3 version string in .rom    -> ...
    E-2.4 where did flash 7.3-2 come from? -> preseed / earlier session / other
    E-3   platform resolved to      -> ES40 / DS20
    E-4   ARMED line emitted?       -> ...
    E-5.A flash absent: banner      -> ...   (= the image's TRUE version)
    E-5.B flash = pre-LFU backup    -> halt at 0x20000000? / unreachable
    E-6.4 A-vs-A diff clean?        -> CLEAN / delta set = ...
    E-6.5 halt PC unchanged w/ trigger armed? -> YES / NO (if NO, snapshot perturbs)
    E-6.2 R-1 image at PA 0x5bc000? -> ...
          R-2 PTE -> 0x5bc000?      -> ...
          R-3 STRANDED (vptb!=0 && vaform_base==0)? -> ...   <-- H2 verdict
    S-A   P00 snapshot strings banner -> ...  (= image version; retires E-5.A)
