...
## 03:06 — Reservation-home identity resolved: three homes, only CpuState wired; settling SSOT wiring tonight
- **Working on:** Same "Readiness check" session, Phase 3. Before greening the `LockArbiter` test, ran an identity review of *where the LL/SC reservation actually lives* in the live tree. Now converging on the design call: which structure is the wired SSOT, and whether the arbiter wraps or deprecates `LockMonitor` — user wants it settled and wired tonight.
- **Done since last checkpoint:**
  - **Three reservation "homes" disambiguated** (was assumed two): (1) **`CpuState` fields** `reservedCacheLine`/`hasReservation` — the ONLY one wired today; `MemDrainer.h` ~620-656 sets them inline on `LDx_L` (`reservedCacheLine = pa & kCacheLineMask`) and check-and-clears on `STx_C`, clearing `hasReservation` regardless of outcome. **No cross-CPU clear on a plain store anywhere in MemDrainer** — correct for single-CPU, fatal for SMP. (2) **`memoryLib::LockMonitor`** (`GuestMemory m_locks`, via `lockMonitor()`) — the designated SSOT: per-CPU `kMaxCPUs=4`, 64-byte line, `clearLine(pa, exceptCpu=-1)` cross-CPU invalidation hook already coded — **but DARK: zero production callers** across the whole tree (only its own def in GuestMemory.h, CpuState.h, journals/docs). (3) **`LockArbiter`** — harness-only (`schedLib/SmpHarness.h` + its test); no production source outside those.
  - **Key correction to the standing assumption:** the 2026-05-14 "reservation migrated into `lockMonitor()`" is **NOT done** — `LockMonitor` is wired nowhere; MemDrainer still uses the CpuState fields. So "step 2 (real-CPU LL/SC)" ≡ "finish the 2026-05-14 migration" — **same task**: route MemDrainer (+ BreakpointSink + CpuStateDump) through `lockMonitor()` and retire the CpuState fields.
  - **Snapshot coupling confirmed:** deleting `reservedCacheLine`/`hasReservation` forces a `kCpuStateVersion` bump and changes `checkCpuStateEqual`/`populateMachine` (round-trip hive ~lines 200-201 / 120-121). The "where does the reservation live" decision *is* the snapshot-version decision — settle together. (Note: per the 23:06 T5 finding, CpuState round-trips as a raw `sizeof(cpu)` blob, so any field removal is already a layout/version change.)
  - **Determinism-staging wrinkle flagged:** the arbiter isn't just a redundant table — it's the determinism-staging mechanism. `step()` must touch only private state + outbox; shared lock state may mutate only single-threaded in `syncPhase`. If MemDrainer calls `lockMonitor().set/clearLine` inline inside `step()`, it mutates shared state during the concurrent phase and breaks `determinism_equivalence` under `ThreadedDriver`. Synthesis proposed: **LockMonitor = SSOT table; the staged-Effect discipline must survive** — reservation op staged in `step()`, applied to LockMonitor in `syncPhase`, arbiter collapsing to a thin forwarding adapter over `lockMonitor()`.
- **Open / next (the piece being settled tonight):**
  1. User's framing: **SSOT need not be LockMonitor** — "we just need to ensure the SSOT is wired tonight." Decision on the table: **wrap `LockMonitor` inside `LockArbiter` (adapter)** vs **deprecate `LockMonitor`** and let the arbiter (or CpuState path) be the wired SSOT. Pick one and wire it this session.
  2. Whatever is chosen, the wiring must preserve the staged-Effect discipline (mutate shared reservation state only in `syncPhase`) so `determinism_equivalence` stays bit-identical, and must be paired with the `kCpuStateVersion` bump if the CpuState fields are retired.
- **Watch-outs:**
  - **Discrepancy to resolve before trusting "STQ_C already works":** user cited a May-2026 TODO that reservation is already in `lockMonitor()` with every store calling `clearLine(pa)`; the session's grep shows `LockMonitor` has **zero production callers** (dark). One of these is stale — verify the actual MemDrainer/store path on a client build before assuming SMP-eventual STQ_C is correct.
  - Do **not** let `LockArbiter` become a permanent third reservation home — it must end as adapter/staging over the chosen SSOT, not its own table.
