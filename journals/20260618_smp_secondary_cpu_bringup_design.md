...
> **FILED 2026-06-18 from an uploaded design spec. EDITOR'S NOTE (same-session
> resolution):** This spec was written while it was still open whether the DS20
> `0xBFFC == 0xCAFEBEEF` poll was a secondary-CPU rendezvous. Its **Prerequisite
> P1 is now ANSWERED**: the apisrm source `apisrm/ref/pc264.c` `platform()` shows
> `if (*(int*)0xBFFC == 0xCAFEBEEF) platform = ISP_MODEL; else REAL_HW;` (comment
> "ISP flag at 0xBFFC … deposit 0xBFFC with CAFEBEEF"). So `0xBFFC=0xCAFEBEEF` is
> the firmware's **ISP-model (pre-silicon SIMULATOR) detection flag, NOT a
> secondary rendezvous**, and it has no writer in the image (harness-deposited).
> Therefore, per this spec's own "Relationship to the DS20 unblock":
> - **DS20 boot to `>>>` = Goal A**: EmulatR claims `ISP_MODEL` (read-intercept
>   `0xBFFC -> 0xCAFEBEEF`; faithful, EmulatR IS an ISP model). NO second CPU.
> - **This SMP spec = Goal B** (two real Alphas under an SMP guest): a separate,
>   larger task, NOT required to reach `>>>`.
> **P2 is partly resolved**: the RSCC/CALL_PAL decode found the console runs the
> **VMS** personality (`ev6_vms_callpal.mar`, `PAL_FUNC__RSCC=^x9D`). Everything
> else in the spec stands. AXPBox boots the REAL_HW path (does not set the ISP
> flag) and may not actually execute CPU1 (verify before using as an SMP oracle).
...
[EDITOR'S NOTE: NOW CONFIRMED -- it is the ISP-model flag (Goal A), SMP-irrelevant.]
...
- **P1 (blocks Phase 5): Confirm and capture the secondary handshake.**
  ...
  [EDITOR'S NOTE: ANSWERED -- pc264.c platform() reads it as the ISP-model flag;
  no writer in image (harness-deposited). => "revisit Goal A": claim ISP_MODEL.]
...
- **P2 (blocks Phase 5, informs Phase 1): Resolve PAL personality.**
  ...
  [EDITOR'S NOTE: leaning VMS (PAL_FUNC__RSCC=^x9D in ev6_vms_callpal.mar). Note
  the PC264 IPL table we used for the IPI work was the OSF pc264 PAL -- reconcile
  OSF-vs-VMS for the CONSOLE image before trusting IPL/PCS layout.]
...
- **P4 (blocks Phase 4): Pin IPI delivery values from PAL decode.**
  ...
  [EDITOR'S NOTE: EI[3]=1<<36 confirmed from the OSF PC264 PAL IPL table; gate =
  canAcceptInterrupt(21) -> IER bit 36. Re-confirm against the VMS console PAL.]
...
- Rendezvous responder: CPU1's init MUST reproduce the EXACT multi-step
  ...
  [EDITOR'S NOTE: P1 shows 0xBFFC is the ISP flag, NOT the secondary handshake.
  The real secondary check-in (start_secondary @ pc264.c:333 = outtig
  0xC00028+id; entry.c secondary_start @425) is the protocol to capture here.]
...
Phase 3 interlock.
[EDITOR'S NOTE: P1 done -- 0xBFFC is the ISP flag, so this SMP work does NOT
touch the DS20 boot hang. Tackle SMP as Goal B on its own merits, not as the
DS20 unblock.]
